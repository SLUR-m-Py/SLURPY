...
x_help = "Number of intra chromosomal contacts to randomly sample."
y_help = "Number of inter chromosomal contacts to randomly sample."
...
def getintra(nlines:int,samplesize:int,intraonly=np.array([])) -> np.ndarray[int]:
...
## Ftn for precalculating random index
def getinter(inter:np.ndarray[int],samplesize:int) -> np.ndarray[int]:
    """Calculates an array of random integers given the total number of lines (nlines) and the sample size."""
    ## If intra only was passed remove from full index and return random numpy array from nlines, given the sample size 
    return np.random.choice(inter, samplesize, replace = replacement)
...
def writesubsample(infile:str, outfile:str, nintra:int, ninter:int, csize:int, saveheader=False, saveindex=False) -> bool: 
    ...
    intraix = getintra(linecount,nintra,intraonly=inter_ix)
    interix = getinter(inter_ix,ninter)
    ...
    with pd.read_csv(infile, sep = hicsep, header = hicheader, names = hiccols, chunksize=csize) as datachunks:
        ...
        for i,chunk in enumerate(datachunks):
            ...
            boolindex = chunk.index.isin(intraix) | chunk.index.isin(interix)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-x", "--intra-down",  dest="x", type=int,   required=True,  help = x_help, metavar = "n")
    parser.add_argument("-y", "--inter-down",  dest="y", type=int,   required=True,  help = y_help, metavar = "n")
    ...
    intra_count = inputs.x
    inter_count = inputs.y 
    ...
    ## set n_sample
    n_sample = intra_count + inter_count
    ...
    assert writesubsample(input_path, output_path, intra_count, inter_count, c_size)
## End of file     