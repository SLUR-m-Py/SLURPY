...
def countlines(infile:str) -> int:
    ...
    with (gzip.open(infile, mode = 'rb') if isgzip(infile) else open(infile, mode = 'r')) as infile:
...
def writesubsample(infile:str, outfile:str, nsample:float, csize=chunks, saveheader=False, saveindex=False) -> bool: 
    ...
    indices = getrandom(linecount,getsamplesize(nsample,linecount))
