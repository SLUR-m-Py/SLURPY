...
def postfilter(inmapping,filedict,setdistance,mapq,chrdict):
    ## add mapped flags
    ...
    ## Check the pairing
    oddling = missingmate(pairmap)
    
    ## Remove any odd records 
    if len(oddling): ## Write set to file
        writeset(filedict['oddling'],oddling,mode='a' if pathexists(filedict['oddling']) else 'w') 
        ## Remove these recoreds 
        pairmap = pairmap[~(pairmap.Qname.isin(oddling))] 
    else: ## Otherwise pass 
        pass 
    ...
    ## If there are records that are of low quality 
    if len(lowqual_reads): ## Write set to file
        writeset(filedict['lowqual'],lowqual_reads,mode='a' if pathexists(filedict['lowqual']) else 'w') 
        ## Filter out the recoreds with matching read names
        pairmap = pairmap[~(pairmap.Qname.isin(lowqual_reads))] 
    else: ## Just pass the pairs map off to the high qualit map 
        pass 
        
    ## apply distance threshodling on intra-chromosomal contacts
    small_tlen = set(pairmap[(pairmap.Rnext=='=')&(pairmap.Tlen.abs()<setdistance)].Qname) if setdistance else []
    
    ## If there are any records failing this filter
    if len(small_tlen): ## Write set to file
        writeset(filedict['distance'],small_tlen,mode='a' if pathexists(filedict['distance']) else 'w') 
        ## Filter out the recoreds with matching read names
        pairmap = pairmap[~(pairmap.Qname.isin(small_tlen))] 
    else: ## Just pass the pairs map off to the high qualit map 
        pass 
    ...
    appenddf(long,filedict['filtered']) if long.shape[0] else None
    pass 
...
if __name__ == "__main__":
    ...
    with pd.read_csv(inpath,chunksize=chunk_size,sep='\t',usecols=samnames,dtype=samdict) as samchunks:
        ...
        for inmap in samchunks:
            ...
            postfilter(tomap,savefiledict,set_distance,map_q,chromdict)
        ...
        postfilter(lastc,savefiledict,set_distance,map_q,chromdict) if (not (lastq in set(tomap.Qname)) and len(lastc)) else None
## End of file 